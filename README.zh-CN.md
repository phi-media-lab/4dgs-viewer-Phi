...
![相机位姿与归一化时间同时变化的 Corgi 4DGS 预览](docs/assets/remote-frame-corgi-motion.webp)

*Corgi——499,980 个 SH3 Gaussian；相机位姿与归一化时间同时变化，并保留
Player 的真实运行时 HUD。本仓库仅分发这段渲染预览，不分发源视频或
Gaussian asset；权利边界见
